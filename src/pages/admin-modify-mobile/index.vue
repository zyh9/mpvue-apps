<template>
  <div class="admin_modify_mobile">
    ...
    <div class="tel_right" :class="currentTab>0?'right':''">
      <div class="options">
        <p>联系电话</p>
        <input type="text" placeholder="请输入您需要绑定的手机号" maxlength="11" v-model="editTel">
      </div>
      <div class="password">
        <p>验证码</p>
        <input type="text" placeholder="请输入验证码" v-model="editVal" maxlength="4">
        <button class="sms_click" @click="sendSms" :disabled="countdown>0?true:false" :class="countdown>0?'disabled':''">{{countdownInfo}}</button>
      </div>
...
<script>
  export default {
    ...
    watch: {
      ...
      editTel: function(newVal, oldVal) {
        this.editTel = newVal.replace(/[^\d]/g, '');
      },
      editVal: function(newVal, oldVal) {
        this.editVal = newVal.replace(/[^\d]/g, '');
      },
      currentTab: function(newVal, oldVal) {
        wx.setNavigationBarTitle({
          title: this.currentTab == 0 ? '修改绑定手机号' : '绑定新手机号'
        })
      }
    },
    beforeDestroy() { //清除定时器
